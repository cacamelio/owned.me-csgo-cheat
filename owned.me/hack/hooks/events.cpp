...
void events::round_start(i_game_event* evt) 
{
	...
	//reset legit delay.
	legitbot::killDelay = 0;

	g_localanims->resetData();

	g::bClearKillFeed = true;
}
...
void events::player_death(i_game_event* evt) 
{
	...
	//set legit switch delay
	if (pAttacker->Index() == g::pLocalPlayer->Index())
	{
		legitbot::killDelay = interfaces::globals->tick_count + legitbot::legit_settings.kill_delay;
	}
