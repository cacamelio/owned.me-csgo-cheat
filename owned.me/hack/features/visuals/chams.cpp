...
bool chams::Players(BaseEntity* pEntity, bool temp, DrawModelResults_t* pResults, const DrawModelInfo_t& info, matrix_t* pBoneToWorld, float* flFlexWeights, float* flFlexDelayedWeights, const vec3_t& vecModelOrigin, int nFlags)
{
	...
	if (g::pLocalPlayer->TeamNum() != pEntity->TeamNum())
	{
		...
		Color colBacktrack = g_menu.ConvertColor(config.players.backtrack_col);
		...
		if (config.players.visible)
		{
			...
			if (config.players.backtrack && temp)
			{
				if (g::pLocalPlayer->IsAlive() && pEntity->Velocity().length_2d() > 0.1f) {

					auto& log = g_LagCompensation->GetPlayerRecords(pEntity->EntIndex());
					if (!log.empty())
					{
						auto record = g_aimbot->find_oldest_record(pEntity);
						...
						if (record != nullptr)
						{
							...
							pMaterial->set_material_var_flag(material_var_ignorez, true);
							pMaterial->set_material_var_flag(material_var_wireframe, config.players.wireframe ? true : false);
							interfaces::studio_render->set_color_modulation(colBacktrack.Base().data());
							interfaces::studio_render->set_alpha_modulation(colBacktrack.aBase());
							interfaces::studio_render->forced_material_override(pMaterial);
							oDrawModel(interfaces::studio_render, 0, pResults, info, record->m_Matricies[record->m_RotationMode].data(), flFlexWeights, flFlexDelayedWeights, vecModelOrigin, nFlags);

							/*auto record = &legit_records::records[pEntity->Index()];
							...
							for (int t = 0; t < record->size(); t++)
							{
								...
								interfaces::studio_render->set_alpha_modulation(colBacktrack.aBase());
								interfaces::studio_render->forced_material_override(pMaterial);
								oDrawModel(interfaces::studio_render, 0, pResults, info, record->at(t).matrix, flFlexWeights, flFlexDelayedWeights, vecModelOrigin, nFlags);
							}*/
						}
					}
				}
	...
	else if (g::pLocalPlayer == pEntity)
	{
		if (config.desync_chams && temp)
		{
			Color colDesync = g_menu.ConvertColor(config.chams_des_col);

			pMaterial->increment_reference_count();
			pMaterial->set_material_var_flag(material_var_ignorez, false);
			pMaterial->set_material_var_flag(material_var_wireframe, /*config.players.wireframe ? true :*/ false);
			interfaces::studio_render->set_color_modulation(colDesync.Base().data());
			interfaces::studio_render->set_alpha_modulation(colDesync.aBase());
			interfaces::studio_render->forced_material_override(pMaterial);

			oDrawModel(interfaces::studio_render, 0, pResults, info, g_localanims->getDesyncMatrix().data(), flFlexWeights, flFlexDelayedWeights, vecModelOrigin, nFlags);
		}
